...
SFDCAdminHelperControllers.controller('SFDCObjectListController',['$rootScope','myFactory','$http','NgTableParams','$sce',function($rootScope,myFactory,$http,NgTableParams,$sce){
    self = this;
    this.objects =['Init'];
    self.standerdObjects =null;
    this.custObject =null;
    self.customObjects =null;
    self.customSetings =null;
    ...
    myFactory.getObjects().then(function(data){
        self.objects = data;
        self.standerdObjects = data.filter(
          function(e){
            return !e.custom;
          }
        );
        self.customObjects = data.filter(
          function(e){
            return e.custom && !e.customSetting;
          }
        );
        self.customSetings = data.filter(
          function(e){
            return e.custom && e.customSetting;
          }
        );
    ...
    this.showCustomOBjects= function(obj){
      console.log(obj);
      self.tableParams.settings({
            dataset: obj
      });
